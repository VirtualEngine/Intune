...
$Warning_URL = "https://github.com/VirtualEngine/Intune/raw/main/BIOS_Update_Warning.zip"
...
$BIOS_Warning_URL = "https://github.com/VirtualEngine/Intune/raw/main/BIOS_Update_Warning.zip"
				...
				Try
					{
						...
						Write_Log -Message_Type "SUCCESS" -Message "Extracting the new BIOS"																
					}
				Catch
					{
						write-output "Cannot extract BIOS update content"							
...
If(($BIOS_EXE_Extracted -eq $True) -and ($Warning_GUI_Downloaded -eq $True) -and ($Warning_ZIP_Extracted -eq $True) -and ($BIOS_Update_Downloaded -eq $True))
	{
						...
						Catch
							{
								...
								write-output "Cannot stop Bitlocker"																	
								EXIT 1
							}
					}	
				Else
		...
		Else
			{
				Write_Log -Message_Type "INFO" -Message "User cancelled the update process"	
				EXIT 1
			}
	}