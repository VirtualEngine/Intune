				...
				If(!(test-path $LZ4_DLL_Path))
					{
						...
						Catch
							{
								write-output "Cannot download DLL LZ4"
								EXIT 0
							}	
					}
				Else
				...
				If($DLL_Download_Success -eq $True)
					{
						...
						Class Lenovo{
							...
							{
								$result = Invoke-WebRequest -Uri "https://pcsupport.lenovo.com/us/en/api/v4/mse/getAllProducts?productId=" -UseBasicParsing -Headers @{
							...
							hidden [Object[]] GetModelWebResponse($modelGUID)
							{
								...
								$SelectedModelwebReq = Invoke-WebRequest -Uri $DownloadURL -UseBasicParsing -Headers @{
								}
						...
						If($Get_GUID -eq $null)
							{
								...
								If($Get_GUID -eq $null)
									{
										write-output "Model not found or verification problem"	
										EXIT 0
									}		
								Else
									{
						...
						If($Model_Found -eq $True)
							{
								...
								$Get_BIOS_Update = $Get_BIOS_Update.files  | Where {($_.Type -eq "EXE") -and ($_.Priority -eq "Critical")}

                                If ($null -ne $Get_BIOS_Update) {
								    ...
								    If($Get_Current_BIOS_Version -lt $Get_New_BIOS_Version)
									    {
										    ...
										    If($Diff_in_days -gt $BIOS_Delay_Days)
											    {		
												    write-output "($Get_Current_Model_FamilyName) Older than 90 days ($Diff_in_days)"
												    EXIT 1
											    }
										    Else
											    {		
												    write-output "($Get_Current_Model_FamilyName) BIOS date Less than 90 days ($Diff_in_days)"
												    EXIT 0
											    }																					
									    }
								    Else
									    {
										    write-output "No new version found -  Current version: $Get_Current_BIOS_Version ($Get_Current_Model_FamilyName)"
										    EXIT 0
									    }
                                   }
                                 Else
                                    {
                                        write-output "No Critical BIOS update found -  Current version: $Get_Current_BIOS_Version ($Get_Current_Model_FamilyName)"
									    EXIT 0
